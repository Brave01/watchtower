...
| 前端 | Vue 3 + Vue Flow + Vite | SPA，开发模式独立运行（热更新），生产通过 Nginx 反向代理 |
...
│   │   ├── store/                   # 数据存储层（SQLite CRUD + 自动迁移）
│   │   ├── model/                   # 数据模型（常量、告警规则等）
│   │   ├── auth/                    # JWT 认证子系统
│   │   ├── middleware/              # HTTP 中间件（认证 + CORS）
│   │   ├── dashboard/               # 拨测模块（prober/scheduler/collector/ssh_ws）
│   │   ├── logmonitor/              # 日志监控子系统
│   │   └── app/                     # 应用层路由注册（v1/user, v1/dashboard, v1/logmonitor）
│   │
...
└── docs/                            # 项目文档
    ...
    ├── architecture-api.md          # 架构与 API 文档（本文件）
    └── user-guide.md                # 用户手册
```
...
- **失败响应**（401）：
  ...
  ```
- **副作用**：更新数据库中的密码 hash，重新签发 JWT Cookie
...
提供共享静态文件（nav.js、theme.css、login.html 等）。
...
- **查询参数**：`host_id`（必需）、`cred_id`（可选，使用已保存凭据）
...
- **终端 resize**：发送 JSON：
  ...
  ```
- **端口规则**：开发环境通过 Vite 代理（同端口），生产环境通过 `VITE_WS_PORT` 环境变量配置直连后端端口（默认 3972）
...
```go
// 1. 检查请求路径是否在豁免列表中（支持 * 后缀 prefix 匹配）
// 2. 如果豁免 → 直接放行
...
|------|------|
| `/login` | 登录页面（精确匹配） |
| `/api/auth/*` | 登录、登出、Me（prefix 匹配） |
| `/common/*` | 静态文件（prefix 匹配） |
| `/api/health` | 健康检查（精确匹配） |

### 6.5 管理员账号

- **默认账号**：`admin` / `admin`
- **密码存储**：bcrypt 加密，持久化到 SQLite 数据库
- **配置方式**：
  - 环境变量 `ADMIN_USER`：设置用户名
  - 环境变量 `ADMIN_PASSWORD_HASH`：设置密码的 bcrypt 哈希（首次启动写入数据库）
  - 如果未配置，默认使用 `admin` 明文密码自动生成 bcrypt 哈希并写入数据库
- **修改密码**：登录后通过 `PUT /api/auth/change-password` API 修改，自动持久化
...
前端 `Hosts.vue` 中 WebSocket URL 的端口拼接逻辑：

```javascript
const backendPort = import.meta.env.VITE_WS_PORT || '3972'
const wsBase = import.meta.env.DEV ? '' : ':' + backendPort
const wsUrl = 'ws://' + location.hostname + wsBase + '/api/ssh/ws?...'
```

| 环境 | `wsBase` | 说明 |
|------|----------|------|
| 开发环境（Vite） | `''` | 走 Vite 代理（同端口），Vite 转发到后端 |
| 生产环境 | `':3972'` | 直连后端 nginx 的 3972 端口（可通过 `VITE_WS_PORT` 环境变量配置） |
...
1. **环境变量** `PORT` — 最高优先级
2. **配置文件** `configs/config.yaml` → `server.port`
3. **代码兜底** — 默认 3972

### 前端 WebSocket 端口

构建时通过 `VITE_WS_PORT` 环境变量配置（Dockerfile 中默认为 `3972`），前端代码中 `import.meta.env.VITE_WS_PORT` 读取。

| 场景 | WebSocket URL | 说明 |
|------|---------------|------|
| 开发环境（Vite 5173） | `ws://localhost:5173/api/ssh/ws` | Vite 代理转发到后端 |
| 生产环境（3972 端口页面） | `ws://ip:3972/api/ssh/ws` | 直连后端 nginx |

### Nginx 代理配置

nginx.conf 中 `/api/ssh/ws` 的 location 需要配置 WebSocket 升级支持：

```nginx
location /api/ssh/ws {
    proxy_pass http://backend:3972;
