	...
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "GET" {
			...
			for _, rule := range rules {
				...
				// 获取关联 webhook 的限流信息
				whID := rule.WebhookID
				if whID > 0 && deps.WebhookClients != nil {
					if c, ok := deps.WebhookClients[whID]; ok {
						item.RateLimitStats = c.GetRateLimitStats()
					}
				}
				if item.RateLimitStats == nil && deps.Webhook != nil {
					item.RateLimitStats = deps.Webhook.GetRateLimitStats()
				}
				result = append(result, item)
			}

			json.NewEncoder(w).Encode(map[string]interface{}{"rules": result})
			return
		}
		if r.Method == "POST" {
